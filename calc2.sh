...
measure_build() {
  ...
  show_progress "Starting build benchmark for $package_manager"
  
  # Record start time for build
  ...
  # Record end time for build
  build_end_time=$(date +%s.%N)
  build_duration=$(echo "$build_end_time - $build_start_time" | bc)
  ...
  show_progress "Build for $package_manager completed in $build_duration seconds"
  
  # Return results as JSON
  echo "{\"duration\": $build_duration, \"exitCode\": $build_exit_code, \"sizeInMB\": $build_size, \"output\": $(echo "$build_output" | jq -Rs .)}"
}
...
measure_startup() {
  ...
  # Measure build if enabled
  build_data="{}"
  if $RUN_BUILD; then
    build_data=$(measure_build "$package_manager" "$build_command")
  fi
...
EOF
  ...
  # Total duration (install + startup + build if enabled)
  if $RUN_BUILD; then
    build_duration=$(echo "$build_data" | jq -r '.duration')
    total_duration=$(echo "$install_duration + $startup_duration + $build_duration" | bc)
  else
...
if $RUN_BUILD; then
  show_progress "Mode: Development server startup + Production build"
else
  show_progress "Mode: Development server startup only"
fi

# Run benchmarks
yarn_time=$(measure_startup "yarn" "yarn" "yarn dev" "yarn build")
pnpm_time=$(measure_startup "pnpm" "pnpm install" "pnpm dev" "pnpm build")
bun_time=$(measure_startup "bun" "bun install" "bun run dev" "bun run build")

# Create combined JSON results
show_progress "Generating combined results"

# Build summary object based on whether build was run
if $RUN_BUILD; then
  summary='{
    "fastest": {
      "overall": (min_by(.metrics.timing.totalTime) | .packageManager),
      "install": (min_by(.metrics.timing.installTime) | .packageManager),
      "startup": (min_by(.metrics.timing.startupTime) | .packageManager),
      "build": (min_by(.build.duration) | .packageManager)
    },
    "resources": {
      ...
      "smallestBuildSize": (min_by(.build.sizeInMB) | .packageManager)
    },
    "timestamp": "'"$(date -u +"%Y-%m-%dT%H:%M:%SZ")"'",
    "buildIncluded": true
  }'
...
fastest=$(jq -r '.summary.fastest.overall' results/benchmark-results.json)
echo -e "\n\033[1;36mFastest Package Manager: \033[1;33m$fastest\033[0m"

# Display build stats if build was run
if $RUN_BUILD; then
  fastest_build=$(jq -r '.summary.fastest.build' results/benchmark-results.json)
  smallest_build=$(jq -r '.summary.resources.smallestBuildSize' results/benchmark-results.json)
  echo -e "Fastest Build: \033[1;33m$fastest_build\033[0m"
  echo -e "Smallest Build: \033[1;33m$smallest_build\033[0m"
fi
